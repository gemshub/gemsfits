#!/bin/bash
#
# DM: gfshell2deployqtapp.sh - a modified/extended deployqtapp.sh from W.Hallatt 
#
#================================================================================
...
qtfontsdir=$HOME/Qt/5.11.0/Src/qtbase/lib/fonts
...
qtplatformplugin=$HOME/Qt/5.11.0/gcc_64/plugins/platforms/libqxcb.so
...
echo "Copying dependencies for libqxcb.so to " $libsdir
cp /home/dmiron/Qt/5.11.0/gcc_64/lib/libQt5XcbQpa.so.5 $libsdir
cp /home/dmiron/Qt/5.11.0/gcc_64/lib/libQt5Core.so.5 $libsdir
cp /home/dmiron/Qt/5.11.0/gcc_64/lib/libQt5Gui.so.5 $libsdir
cp /home/dmiron/Qt/5.11.0/gcc_64/lib/libQt5DBus.so.5 $libsdir

cp /lib/x86_64-linux-gnu/libexpat.so.1 $libsdir
cp /usr/lib/x86_64-linux-gnu/libxshmfence.so.1 $libsdir
cp /lib/x86_64-linux-gnu/libsystemd.so.0 $libsdir
cp /lib/x86_64-linux-gnu/libbsd.so.0 $libsdir

echo "Linking dependencies for libqxcb.so to " $libsdir
patchelf --set-rpath '$ORIGIN/../libs' $tardir/platforms/libqxcb.so

#for dep in `ldd ./$qtplatformplugindir/libqxcb.so | awk '{print $3}' | grep -v "("`
#do
#  cp $dep $libsdir
#  echo "Copied dependency "$dep" to "$libsdir
#done
...
# requires.
qtsqliteplugin=$HOME/Qt/5.11.0/gcc_64/plugins/sqldrivers/libqsqlite.so
...
qtqcollectiongenerator=$HOME/Qt/5.11.0/gcc_64/bin/qcollectiongenerator
cp $qtqcollectiongenerator $tardir
echo "Copied executable "$qtqcollectiongenerator" to "$tardir
qtqhelpgenerator=$HOME/Qt/5.11.0/gcc_64/bin/qhelpgenerator
