...
UPDATE impdata.temp1 SET solubility = 10^(-temp2.solubility) FROM impdata.temp2 WHERE temp1.id=temp2.id;
UPDATE impdata.temp1 SET id_elem = (SELECT element_id from public.elements e WHERE e.name = 'pH');

DELETE FROM impdata.temp3;
INSERT INTO impdata.temp3 (log_solubility)
SELECT  ph_aq FROM impdata.expresaq;
UPDATE impdata.temp1 SET log_solubility = temp3.log_solubility FROM impdata.temp3 WHERE temp1.id=temp3.id;
...
SELECT id_exp, id_elem, solubility, log_solubility, error, log_error FROM impdata.temp1;
...
UPDATE impdata.temp1 SET solubility = temp2.solubility*(10^(-3)) FROM impdata.temp2 WHERE temp1.id=temp2.id; -- from mM/l to M/l
...
UPDATE impdata.temp1 SET error = 10^temp4.error FROM impdata.temp4 WHERE temp1.id=temp4.id;
...
UPDATE impdata.temp1 SET solubility = temp2.solubility*(10^(-3)) FROM impdata.temp2 WHERE temp1.id=temp2.id;
...
UPDATE impdata.temp1 SET solubility = temp2.solubility*(10^(-3)) FROM impdata.temp2 WHERE temp1.id=temp2.id;
UPDATE impdata.temp1 SET id_elem = (SELECT element_id from public.elements e WHERE e.name = 'C');
...
DELETE FROM public.results_aq WHERE (solubility, log_solubility) IS NULL; -- deletes empty element fields
