...
{
    if (vect_of_sys_props[0]->fit_std) {
    make_opt_vector( vect_of_sys_props );
    }
    else {
        make_opt_vector_inter( vect_of_sys_props );
    }
}
...
void SS_Opt_Vector::make_opt_vector_inter( vector<SS_System_Properties*> vect_of_sys_props )
{
    ...
    fit_species_inter.push_back(spec1);
    fit_species_inter.push_back(spec2);
...
{
    /// if (vect_of_sys_props[0].) --- check for type of fit here
    make_opt_vector( vect_of_sys_props );
}
