...
SS_System_Properties::SS_System_Properties( )
{
    ...
    fout << "2. system_properties.cpp line 79. Creating new SS_Data_Manager data_meas" << endl;
    ...
    WhatToFit_s = allparam.substr((pos_start+f5.length()),(pos_end-pos_start-f5.length()));
    istringstream WhatToFit_ss(WhatToFit_s);
    for(int i=0; i<1; i++)
    {
        WhatToFit_ss >> sub_WhatToFit;
        fit_std = atoi(sub_WhatToFit.c_str());
    }
...
void SS_System_Properties::getsysprop( std_prop* sysprop )
{
    ...
    for( i=0; i<1; i++)
    {
        ...
        log_solubility = atoi(sub_LogSolubility.c_str());
    }
...
void SS_System_Properties::getparam( parameters* sysparam )
{
    ...
    string LsMod_s, LsMdc_s, SysName_s, PhaName_s, SpecName_s, SpecCharge_s, LogSolubility_s;
    string sub_LsMod, sub_LsMdc, sub_SysName, sub_SpecCharge, sub_PhaName, sub_SpecName, sub_ActName, sub_LogSolubility;
    int pos_start, pos_end;
    unsigned int i;
    ifstream param_stream;
    ...
    // Log Solubility
    pos_start = allparam.find(f12);
    pos_end   = allparam.find(f4,pos_start);
    LogSolubility_s = allparam.substr((pos_start+f12.length()),(pos_end-pos_start-f12.length()));
    istringstream LogSolubility_ss(LogSolubility_s);
    for( i=0; i<1; i++)
    {
        LogSolubility_ss >> sub_LogSolubility;
...
//    fout<<"charges[0] = "<<charges[0]<<" | charges[1] = "<<charges[1]<<" | charges[2] = "<<charges[2]<<endl;
    ...
    string sizes = LsMod_s.c_str();
    int sizess = sizes.size();
    for( i=0; i<sizes.size(); i++)
    {
        sub_LsMod = sizes.at(i);
        if( i==sizes.size()-7 ){	sysparam->rows_aIPc = atoi(sub_LsMod.c_str()); };
        if( i==sizes.size()-5 ){	sysparam->cols_aIPx = atoi(sub_LsMod.c_str()); };
        if( i==sizes.size()-3 ){	sysparam->cols_aIPc = atoi(sub_LsMod.c_str()); };
    }
...
/*	if( ! strncmp( sub_ActName.c_str(), "ELVIS", 20 ) )
    ...
    for( i=0; i<data_meas->allexp.size(); i++ )
    {
        if( (node->check_TP( data_meas->allexp[i]->TC+273.15, 100000 * data_meas->allexp[i]->PG) == false &&  node->check_grid_TP( data_meas->allexp[i]->TC+273.15, 100000 * data_meas->allexp[i]->PG ) < 0. ) /*&& node->check_grid_TP( sysdata->temperature.at(i), 100000 * sysdata->pressure.at(i) ) < 0*/ )
        {
            cout<<endl;
            cout<<" Measurement data: temperature "<<data_meas->allexp[i]->TC<<" together with pressure "<<100000 * data_meas->allexp[i]->PG<<" does not appear in the DATACH structure of GEMS3K !!!!"<<endl;
            //cout<<" bailing out now ... "<<endl;
            cout<<endl;
            //exit(1);
        }
