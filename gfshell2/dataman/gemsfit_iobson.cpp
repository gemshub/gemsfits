...
bool add_unit_and_error(nlohmann::json& exp, std::vector<std::string> headline, std::vector<std::string> row , unsigned int &position);
...
void csvToBson( nlohmann::json& exp, const  std::vector<std::string>& headline, const std::vector<std::string>& row )
{
    ...
    // array of components
    //++ START array sbcomp ++//
    exp[sbcomp] = nlohmann::json::array();
    for (unsigned int i=0; i<headline.size(); ++i)
    {
        cut_csv_header(headline[i], header_delimiter, header_vector);
    ...
    if (h_phase)
    {
    exp[expphases] = nlohmann::json::array(); // going trough the headline and searching for "phase" keyword
    for (unsigned int i=0; i<headline.size(); ++i)
    {
        cut_csv_header(headline[i], header_delimiter, header_vector);
        if ((header_vector[ndx_phase] == phase) && (!row[i].empty()))
        {
            ...
            {
                auto obj_phase = nlohmann::json::object();                                                            // phases counter increased by 1
                ...
                if (h_phprop)
                {
                    obj_phase[phprop] = nlohmann::json::array();
                    ...
                    for (unsigned int j=0; j<headline.size(); ++j)
                    {
                        ...
                        {
                            auto obj = nlohmann::json::object();                                                            // phases counter increased by 1                                      // increase the number of phases properties with one
                            ...
                            obj_phase[phprop].push_back(obj); // END property object
                        }
                    }
                    //++ END array phprop ++//
                } h_phprop = false;
                ...
                if (h_phIC)
                {
                    obj_phase[phIC] = nlohmann::json::array();
                    // get phase comp
                    for (unsigned int j=0; j<headline.size(); ++j)
                    {
                        cut_csv_header(headline[j], header_delimiter, header_vector);
                            ...
                            (header_vector[ndx_phase_name] == phase_name) && (header_vector[ndx_IC] == IC ))
                        {
                            ...
                            obj_phase[phIC].push_back(obj); // END phase element object
                        }
                    }
                    //++ END array phcomp IC ++//
                } h_phIC = false;
                ...
                if (h_phMR)
                {
                    obj_phase[phMR] = nlohmann::json::array();
                    // get phase MR
                    for (unsigned int j=0; j<headline.size(); ++j)
                    {
                        cut_csv_header(headline[j], header_delimiter, header_vector);
                            ...
                            (header_vector[ndx_phase_name] == phase_name) && (header_vector[ndx_MR] == MR ))
                        {
                            ...
                            obj_phase[phMR].push_back(obj); // END phase element object
                        }
                    }
                    //++ END array phMR ++//
                } h_phMR = false;
                ...
                if (h_phDC) // check if there is species data in the CSV header
                {
                    ...
                    obj_phase[phDC] = nlohmann::json::array();
                    for (unsigned int j=0; j<headline.size(); ++j)
                    {
                        cut_csv_header(headline[j], header_delimiter, header_vector);
                            ...
                            if (!dcomp_already_added)
                            {
                                ...
                                for (unsigned int k=0; k<headline.size(); ++k)
                                {
                                        ...
                                        (header_vector[ndx_phase_name] == phase_name) && (header_vector[ndx_DC] == DC ) && (!row[k].empty()))
                                    {
                                        ...
                                        spec_obj[DCprop].push_back(obj);
                                    } // end search for dcomp name in the headline of the csv file
                                }
                                //++ END dcompprop array
                                obj_phase[phDC].push_back(spec_obj);
                ...
                exp[expphases].push_back(obj_phase); //++ END phase object ++
